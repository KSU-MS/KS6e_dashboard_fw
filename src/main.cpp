...
WS2812Serial leds(numled, displayMemory, drawingMemory, NEOPIXELDIN,
                  WS2812_GRB);
...
void loop() {
  ...
  if (send_dash_status_timer.check()) {
    ...
    Serial.printf("dash status hash: %x time: %d dirty: %d main: %d\n",
                  dash_status_t.firmware_version, dash_status_t.on_time_seconds,
                  dash_status_t.project_is_dirty,
                  dash_status_t.project_on_main_or_master);
#endif
    WriteCAN(fw_hash_msg);
  }
  ...
  if (update_fault_leds.check()) {
    // NEED THE ! there so the leds work, pull low instead of high. confirmed
    // working 3/28/23, in VCU false = light ON, true = light OFF
    ...
    // For the inverter fault, we just OR all the fault fields, since fault code
    // > 0 == bad == turn light on
    analogWrite(INVERTER_LED,
                fault_led_duty * (mc_fault_codes.get_post_fault_hi() ||
                                  mc_fault_codes.get_post_fault_lo() ||
                                  mc_fault_codes.get_run_fault_hi() ||
                                  mc_fault_codes.get_run_fault_lo()));

    analogWrite(MISCLED3,
                fault_led_duty * (vcu_status.get_accel_implausability()));
    analogWrite(MISCLED1,
                fault_led_duty * (vcu_status.get_accel_brake_implausability()));
    analogWrite(MISCLED2,
                fault_led_duty * (vcu_status.get_brake_implausibility()));
#if DEBUG
    ...
    Serial.printf(
        "These are the Inverter Fault Codes: Post_fault_hi: %d Post_fault_lo: "
        "%d Run_fault_hi: %d Run_fault_lo: %d\n",
        mc_fault_codes.get_post_fault_hi(), mc_fault_codes.get_post_fault_lo(),
        mc_fault_codes.get_run_fault_hi(), mc_fault_codes.get_run_fault_lo());
    Serial.printf(
        "Accel implaus: %d Accel&Brake Implaus: %d Brake Implaus: %d\n",
        vcu_status.get_accel_implausability(),
        vcu_status.get_accel_brake_implausability(),
        vcu_status.get_brake_implausibility());
#endif
  }
}
...
bool dash_init() {
  ...
  seven_segment.writeDisplay();
  delay(500);
  vcu_status.set_pedal_states(
      0xff); // Set pedal states to fault state so lights are on if SNA
  return true;
};
...
void updateSOCNeopixels(int soc) {
  ...
  Serial.printf("Set %d to %d ON, set %d to %d OFF\n", 1, num_leds_enabled,
                num_leds_enabled + 1, PIXELS_FOR_SOC);
#endif
}
/**
 * @brief
...
void set_single_segment_indicator(uint8_t number_to_display) {
  ...
  Serial.printf("SEVEN SEGMENT INPUT: %d A: %d B: %d C: %d D: %d\n",
                number_to_display, led_a_high, led_b_high, led_c_high,
                led_d_high);
#endif
